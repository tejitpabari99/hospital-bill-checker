# Step 17: /data Page — Data Sources Reference

> **AGENT INSTRUCTIONS:** You are implementing step 17.
> Work in `/root/projects/hospital-bill-checker`. Steps 00–16 must be complete.
> Read `next-steps/README.md` for full project context.

**Goal:** Create a `/data` SvelteKit route that lists every data source the app uses: what it is,
when it's refreshed, the CMS download URL, the local SQLite file, the build script, and the columns
the app actually uses. This is a transparency/reference page — no server-side fetch needed.
...
- [ ] Create directory `src/routes/data/` if it does not exist

- [ ] Create `src/routes/data/+page.ts`:

```typescript
export const prerender = true
```

- [ ] Create `src/routes/data/+page.svelte` with:
...
- [ ] In `src/routes/+layout.svelte`, add a link to `/data` near the other nav links:
...
## Task 3: Run dev server and verify

- [ ] `npm run dev -- --port 5173`
- [ ] Open `http://localhost:5173/data`
- [ ] Verify all 11 data sources render correctly
- [ ] Verify summary cards show
- [ ] Verify CMS URLs are clickable
...
git add src/routes/data/+page.svelte src/routes/data/+page.ts
git commit -m "feat: add /data page listing all CMS data sources with refresh schedules and column usage"
```
