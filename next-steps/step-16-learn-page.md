# Step 16: Learn Page — Patient Education with Simple/Detailed Toggle

> **AGENT INSTRUCTIONS:** You are implementing step 16.
> Work in `/root/projects/hospital-bill-checker`. Steps 00–15 must be complete.
> Read `next-steps/README.md` for full project context.

**Goal:** Create a new `/learn` route that explains every check the app performs. Two modes:
- **Simple** — plain-English, one paragraph per check, no jargon
- **Detailed** — technical detail, code references, links to CMS data sources

Mode is toggled by a button in the top-right corner. Default is Simple. State is client-side only
(no server round-trip). The page must be fully mobile-responsive at 375px width.

The page also includes two diagram placeholder sections (the user will supply the actual images later).

**Files to create:**
- `src/routes/learn/+page.svelte` — full page
- `src/routes/learn/+page.ts` — minimal preload (no server fetch needed)

---

## Task 1: Create the route skeleton

- [ ] Create directory `src/routes/learn/` if it does not exist
- [ ] Create `src/routes/learn/+page.ts`:

```typescript
export const prerender = true
```

- [ ] Create `src/routes/learn/+page.svelte` with this skeleton:
...
## Task 2: Add the checks data array

Inside the `<script>` block, add this array after the `mode` declaration:
...
<p><strong>Note:</strong> MUE values are published without the actual numeric limits in public files for some codes (marked "N/A"). Those codes are not checked.</p>`,
    },
    {
      ...
      detailed: `<p><strong>Data source:</strong> Medicare Physician Fee Schedule (MPFS) — <code>PPRRVU26.xlsx</code> from <a href="https://www.cms.gov/medicare/physician-fee-schedule/search/overview" target="_blank">CMS MPFS downloads</a>. Non-facility rate calculated as <code>NONFAC_TOTAL × 33.29</code> (2026 conversion factor).</p>
...
<p><strong>Stage 2 (future):</strong> Location-adjusted rates using GPCI (Geographic Practice Cost Indices) from <code>rvu26b.zip</code> — files <code>GPCI2026.xlsx</code> and <code>26LOCCO.xlsx</code>. Not yet implemented.</p>`,
    },
    {
      ...
      detailed: `<p><strong>Data source:</strong> CMS Clinical Laboratory Fee Schedule (CLFS) — annual CSV/TXT from <a href="https://www.cms.gov/medicare/payment/fee-schedules/clinical-laboratory" target="_blank">CMS CLFS page</a>. Delimited file with varying format; column detection is adaptive (scans first 5 rows for header containing "HCPCS").</p>
...
<p><strong>Code:</strong> <code>src/lib/server/audit-rules.ts</code> → <code>checkClfsBenchmark()</code>. DB loader: <code>loadClfsRate()</code>.</p>`,
    },
    {
      ...
      detailed: `<p><strong>Data source:</strong> CMS Average Sales Price (ASP) — quarterly Excel files from <a href="https://www.cms.gov/medicare/payment/part-b-drugs/average-sales-price" target="_blank">CMS ASP page</a>. Accepts J-codes, Q-codes, C-codes, A-codes, and B-codes. Header row detection scans first 15 rows for "HCPCS CODE".</p>
...
<p><strong>Future:</strong> NDC-to-HCPCS crosswalk (<code>asp_ndc_hcpcs_crosswalk</code> table is already created, schema only) to match drug bills that use NDC codes instead of HCPCS.</p>`,
    },
    {
      ...
      detailed: `<p><strong>Data source:</strong> CMS OPPS Addendum B (HCPCS → APC → payment rate) and Addendum A (APC reference data). Downloaded from <a href="https://www.cms.gov/medicare/payment/prospective-payment-systems/hospital-outpatient" target="_blank">CMS OPPS page</a>. Column detection is flexible (column index discovery per file).</p>
...
<p><strong>Only applies to:</strong> Bills classified as <code>outpatient</code> by the document classifier (step 12).</p>`,
    },
    {
      ...
      detailed: `<p><strong>Data source:</strong> CMS IPPS DRG weights table — Excel file from <a href="https://www.cms.gov/medicare/payment/prospective-payment-systems/acute-inpatient-pps" target="_blank">CMS IPPS page</a>. Sheet detection is adaptive (scans all sheets for "DRG" in header row). DRG codes are zero-padded to 3 digits.</p>
...
- [ ] Open `src/routes/+layout.svelte` (or wherever the site nav lives)
- [ ] Add a link to `/learn` in the navigation, e.g.:

```html
<a href="/learn">Learn</a>
```

Place it near the existing `/how-it-works` link.

---

## Task 5: Run dev server and verify

- [ ] `npm run dev -- --port 5173`
- [ ] Open `http://localhost:5173/learn`
- [ ] Click "Detailed" toggle — verify detailed content appears with code refs and links
- [ ] Click "Simple" toggle — verify plain text shows
- [ ] Resize to 375px — verify layout doesn't overflow
- [ ] Stop dev server
...
git add src/routes/learn/+page.svelte src/routes/learn/+page.ts
git commit -m "feat: add /learn page with simple/detailed toggle and all 13 check explanations"
```
